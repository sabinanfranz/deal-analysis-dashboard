...
selected_team = st.sidebar.selectbox("팀을 선택하세요", TEAM_LIST)

df_team = df[df['팀'] == selected_team]
...
# ────────── 테이블들 ──────────
size_tbl = pivot_sum(df_team, '기업 규모').reindex(SIZE_ORDER)

tbl_ent = reorder_types(pivot_sum(df_team[df_team['기업 규모']=='대기업'],  '딜 전환 유형'))
tbl_mid = reorder_types(pivot_sum(df_team[df_team['기업 규모']=='중견기업'], '딜 전환 유형'))
tbl_sme = reorder_types(pivot_sum(df_team[df_team['기업 규모']=='중소기업'], '딜 전환 유형'))

# ────────── 출력 ──────────
# 헤더 크기 축소—h2, h3 사용
st.markdown(f"## {selected_team} 체결액 추이 (2025년)")

st.markdown("### 1. 기업 규모별 체결액 – 월별 (억)")
