# pages/12_사업부_온라인_체결률(리텐션).py
"""
사업부 온라인 – 2025년 월별 체결률 (리텐션 고객)
- 1월 ~ 12월 월별 집계
- 3개월 이동평균(1~3 … 10~12월)
...
st.set_page_config(page_title="사업부-온라인 – 월별 체결률 (2025·리텐션)", layout="wide")

# ────────── 팀 매핑 ──────────
TEAM_RAW = {
    '기업교육 1팀': ['김별','김솔이','황초롱','김정은','김동찬','정태윤','서정연',
                 '강지선','정하영','하승민','이은서','홍제환','김세연','오진선', '박범규'],
    '기업교육 2팀': ['박종협','정선희','권노을','강진우','이윤지','김민선','이수빈','강연정','강다현','방신우','정다혜','임재우','송승희','손승완','김윤지','손지훈','홍예진', '이현진'],
}
...
# ✅ 필터: 2025년, 1~12월, 온라인 포맷만 포함, 리텐션 고객만
df = df[(df['생성년도'] == 2025) &
        (df['생성월'].between(1, 12)) &
...
# ────────── UI ──────────
st.title("사업부 온라인 — 2025년 월별 체결률 (리텐션 고객)")
