...
DISPLAY_COLS = [
    ...
    "수주 예정일","계약 체결일","예상 체결액","금액","수강시작일","수강종료일",
    "코스 ID","카테고리","과정포맷"      # ← 코스 ID 뒤에 카테고리/과정포맷
]
...
# 🔧 (핵심 수정) 금액/예상 체결액은 컬럼이 이미 있어도 항상 숫자형으로 강제 정규화
...
R["R7"] = mask_both & later_than_start & (~same_year_month)
...
# 공통 표시용 DF (안전: 이슈수/코드/설명 없으면 즉시 생성)
