...
# ────────── 0. 페이지 설정 ──────────
st.set_page_config(page_title="팀별 체결률 (2025 · 1-6)", layout="wide")
...
df['온라인여부']   = df['과정포맷(대)'].apply(
    lambda x: '온라인' if x in ONLINE_SET else '비온라인'
)
...
sel_member  = st.sidebar.selectbox("🧑‍💼 담당자 (딜 목록 필터)", ["전체"] + members)

team_df     = df[df['팀'] == sel_team]

# ────────── 4. 공통 마스크 ──────────
succ_mask = team_df['성사 가능성'].isin(['확정', '높음'])
on_mask   = team_df['온라인여부'] == '온라인'
off_mask  = ~on_mask                                   # 비온라인
...
# ────────── 5. 헬퍼 ──────────
def tot_rate(base_mask, extra_mask):
...
def detail_rows(base_mask, extra_mask):
    rows = team_df[base_mask & extra_mask & member_mask][DETAIL_COLS].copy()
...
def render_section(idx: int, title: str, base_mask: pd.Series):
    ...
    for m in members:
        ...
        off_tot, off_rate = tot_rate(base_mask, off_mask & msel)
        on_tot , on_rate  = tot_rate(base_mask,  on_mask & msel)
        summary.append([m, off_tot, off_rate, on_tot, on_rate])

    st.markdown(f"### {num}) {title} – 체결률")
    st.dataframe(
        pd.DataFrame(summary,
            columns=['담당자','비온라인 전체 딜 개수','비온라인(%)','온라인 전체 딜 개수','온라인(%)']
        ).set_index('담당자'),
        use_container_width=True, hide_index=False
    )

    # 5-2) 상세 표 a – 비온라인
    st.markdown(f"#### {num}-1) {title} 비온라인 딜 목록")
    st.dataframe(
        detail_rows(base_mask, off_mask),
        use_container_width=True, hide_index=True
    )

    # 5-3) 상세 표 b – 온라인
    st.markdown(f"#### {num}-2) {title} 온라인 딜 목록")
    st.dataframe(
        detail_rows(base_mask, on_mask),
        use_container_width=True, hide_index=True
    )

# ────────── 6. 출력 ──────────
st.title("개인별 체결률 - '25 1~6월")
