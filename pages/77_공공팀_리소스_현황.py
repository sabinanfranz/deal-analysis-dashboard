# pages/21_공공교육팀_운영리소스_및_성사가능성.py
"""
공공교육팀 — 운영 리소스 & 성사 가능성 (2025, 일간/주간)
- [리소스] won + all(확정 & 수강시작/종료 유효 & 수주예정액>0), won-중복 제거, ONLINE 제외, 2025 경계 클리핑
- [성사가능성] 상태= {높음, 낮음, 미기재, LOST}. 리소스 현황= 높음+낮음+미기재, 중견중소 현황 유지
- 구성: (단일 탭) 공공교육팀
  · 담당자별 일간 스택 막대 + 주간 합계 표(팀원 전원)
  · 요약 표(담당자, '예상 체결액'(억원) 행 포함)
  · 개인 필터 + 운영 리소스 산정 딜 목록(표)
  · 성사 상태별 상세표(높음/낮음/미기재/LOST)
"""
...
# ─────────────────────────── 공통 설정/상수
st.set_page_config(page_title="공공교육팀 — 운영 리소스 & 성사 가능성 (2025)", layout="wide")
...
TEAMS = list(TEAM_RAW.keys())  # ['공공교육팀'] 만
...
def _weight_from_amount(won):
    ...
    if x <=   5_000_000:   return 0.5
    if x <=  25_000_000:   return 1.0
    if x <=  50_000_000:   return 1.5
    if x <= 100_000_000:   return 2.0
    if x <= 300_000_000:   return 2.5
    return 3.0
...
def _weekly_sum(daily_df):
    """일간 피벗을 9개 주(월~일)로 합산. 행=주차, 열=카테고리. (소수 1자리 유지)"""
...
def _prepare_resource_rows():
    ...
    - won: 금액(우선) or 수주예정액(종합)으로 weight 산정 (금액 미기재면 **최소 가중치 0.5**)
    ...
    won = won[won['팀'].isin(TEAMS)].copy()  # 공공교육팀만
    ...
    alld = alld[alld['팀'].isin(TEAMS)].copy()  # 공공교육팀만
...
def detail_df(team_like_df: pd.DataFrame, persons: list[str] | None, status: str) -> pd.DataFrame:
    """상세 표(상태별). persons=None이면 전 팀, 리스트면 해당 담당자만.
       수주 예정액(종합): 억 단위(두 자리 소수) 문자열 표기, 아래 중 하나라도 해당되면 '미기재':
    ...
    sub['name_rank'] = sub['담당자_name'].map(
        lambda n: TEAM_RAW.get('공공교육팀', []).index(n)
        if n in TEAM_RAW.get('공공교육팀', []) else 999
    )
    sub = sub.sort_values(['name_rank','생성월','기업명'])

    # 표시 컬럼 보강
    for c in DETAIL_COLS:
        if c not in sub.columns:
            sub[c] = pd.NA
    res = sub.drop(columns=['name_rank'])[DETAIL_COLS].copy()
    ...
    if '수주 예정액(종합)' in res.columns:
        ...
        missing_or_zero = is_blank | num.isna() | (num <= 0) | (null_amount & null_expected)
...
def make_resource_selection_table(res_rows: pd.DataFrame, team: str, persons_sel: list[str]) -> pd.DataFrame:
    ...
    view = sub.copy()
    # 수주 예정액(종합) — 억 단위 문자열(두 자리), 값 없으면 '미기재'
    if '수주 예정액(종합)' in view.columns:
        ...
        view['수주 예정액(종합)'] = np.where(num.notna() & (num > 0),
                                          ...
                                          '미기재')
    # 일별 가중치 — 소수 1자리
    view['일별 가중치'] = view['weight'].round(1)
...
# ─────────────────────────── 데이터 준비
st.title("공공교육팀 — 운영 리소스 & 성사 가능성 — 2025")

with st.expander("정책 요약 / 계산 기준", expanded=False):
    st.markdown("""
- **리소스 포함 기준**
  - **WON**: 팀 소속(**공공교육팀**), ONLINE 제외, `수강시작일~수강종료일` **유효**(2025 경계로 클리핑)  
    금액 열(우선순위 **금액 → 수주 예정액(종합) → 계약금액 → 수주금액 → 총금액**)로 가중치 산정  
    *(금액 미기재/0이어도 **최소 가중치 0.5**로 반영)*  
...
|:---------------------------------|:-----------:|
| ≤ 5,000,000                      | 0.5 |
| 5,000,001 ~ 25,000,000           | 1.0 |
| 25,000,001 ~ 50,000,000          | 1.5 |
| 50,000,001 ~ 100,000,000         | 2.0 |
| 100,000,001 ~ 300,000,000        | 2.5 |
| > 300,000,000                    | 3.0 |
...
# 공공교육팀 — 담당자별 일간 리소스
rows_pub = res_rows[res_rows['팀'] == '공공교육팀']
p_daily = _accumulate_by_key(rows_pub, '담당자_name')

# 팀원 전원 포함(값 없으면 0으로 채움)
persons_all = TEAM_RAW['공공교육팀']
p_daily = p_daily.reindex(columns=persons_all).fillna(0.0)

# 팀 총 리소스(연간 합)
m_pub = float(p_daily.sum(axis=1).sum())

# [성사가능성] 준비
s_df = _prepare_status_df()
s_team = s_df[s_df['팀'] == '공공교육팀'].copy()

# ─────────────────────────── 단일 탭: 공공교육팀
tab_pub, = st.tabs(["공공교육팀"])

with tab_pub:
    # (리소스) ─────────────────
    st.subheader("담당자별 일간 스택 막대")
    if not p_daily.empty and p_daily.shape[1] > 0:
        c1, = st.columns(1)
        c1.metric("공공교육팀 총 리소스(연간 합)", f"{m_pub:,.0f}")
        p_long, p_order_chart = _melt_with_order(p_daily, var_name="담당자")
        st.altair_chart(
            _stacked_bar(p_long, color_col="담당자", title="공공교육팀 — 담당자별 일간 스택 막대", color_order=p_order_chart),
            use_container_width=True
        )
    else:
        st.info("공공교육팀 데이터가 없습니다.")

    # 주간 표(팀원 전원 + 열 순서 산출)
    st.markdown("### 주간 리소스 합계 (금주 기준 -4주 ~ +4주)")
    st.caption(f"금주(월~일): {WEEK_LABELS[4]} · 범위: {WEEK_LABELS[0]} → {WEEK_LABELS[-1]}")
    tbl_pub_wk, order_pub = _weekly_with_order(p_daily, persons_all)  # 팀원 전원 포함 + 정렬
    st.dataframe(tbl_pub_wk, use_container_width=True)

    # (성사가능성) ─────────────
    st.markdown("---")
    st.subheader("요약 표 (x=담당자 / y=카운트 항목) — 팀원 전체")
    mat_pub = summary_matrix_people(s_team, '공공교육팀', col_order=order_pub)  # 열 순서 동기화
    st.dataframe(mat_pub, use_container_width=True)

    st.markdown('---')
    sel = st.selectbox('담당자 필터', ['전체'] + persons_all, 0, key='pub_filter')
    persons_sel = persons_all if sel == '전체' else [sel]

    # 운영 리소스 산정 딜 (담당자 필터 적용)
    st.markdown("### 운영 리소스 산정 딜 (담당자 필터 적용)")
    res_tbl = make_resource_selection_table(res_rows, '공공교육팀', persons_sel)
    st.dataframe(res_tbl, use_container_width=True, hide_index=True)

    # 상태 상세 표들
    SHOW_STATUS = ['높음','낮음','미기재','LOST']
    for stt in SHOW_STATUS:
        st.markdown(f"#### {stt} 상세")
        sub = detail_df(s_team, persons=persons_sel, status=stt)
